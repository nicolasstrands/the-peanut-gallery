...
final class OverlayView: NSView {
    private struct Particle { var emoji: String; var x: CGFloat; var y: CGFloat; var age: CGFloat; let speed: CGFloat; let drift: CGFloat; let size: CGFloat }
    ...
    private func tick() {
        particles = particles.compactMap { particle in
            var item = particle; item.age += 1; item.y += item.speed; item.x += sin(item.age / 18) * item.drift
